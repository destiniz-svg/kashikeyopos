/* End-to-end smoke test: till pairing → catalog push → guest ordering → KDS flow → waiter call */
const B = process.env.BASE || "http://127.0.0.1:4000";
const j = (r) => r.json();
(async () => {
  const reg = await fetch(B + "/api/register", { method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email: "abdulla@steva.mv", password: "test1234", storeName: "Island Zone Café" }) }).then(j);
  console.log("1 register:", reg.slug, reg.register);
  const H = { "Content-Type": "application/json", Authorization: "Bearer " + reg.token };
  const op = { opId: "op1", puts: [
    { kind: "settings", id: "settings", data: { storeName: "Island Zone Café", gstBp: 800, currency: "MVR" } },
    { kind: "products", id: "p1", data: { id: "p1", name: "Espresso", emoji: "☕", cat: "Coffee", price: 3500, cost: 900, stock: 50, unit: "cup" } },
    { kind: "zones", id: "z1", data: { id: "z1", name: "Malé", fee: 2000, eta: "30–45 min" } },
  ...
  const reg2 = await fetch(B + "/api/login", { method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email: "abdulla@steva.mv", password: "test1234" }) }).then(j);
  ...
  const ord = await fetch(`${B}/p/${reg.slug}/order`, { method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ items: [{ pid: "p1", qty: 2 }], table: "T1", custId: "c1", gtype: "table" }) }).then(j);
  ...
  await fetch(B + "/api/ops", { method: "POST", headers: H, body: JSON.stringify({ ops: [
    { opId: "op-status", puts: [{ kind: "orders", id: got.id, data: { ...got.data, status: "preparing" } }] }] }) });
  const gv = await fetch(`${B}/p/${reg.slug}/orders?c=c1`).then(j);
  console.log("8 guest sees live status:", gv.orders[0].status);
  await fetch(`${B}/p/${reg.slug}/call`, { method: "POST", headers: { "Content-Type": "application/json" },

...
test('empty database — every screen renders and no handler throws', () => {
  ...
  assert.strictEqual(r.m.rendered, 37, 'all 37 modal kinds open');
  assert.strictEqual(r.f.rendered, 71, 'all 71 forms have a spec');
...
test('seeded outlet — every screen renders and no handler throws', () => {
  ...
  assert.strictEqual(r.m.rendered, 37);
});

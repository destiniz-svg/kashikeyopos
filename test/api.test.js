...
/* ═══ PER-TICKET VERSIONS (slice 2.2, Scenario G) ═══════════════════════════
   ...
   `ticket.party_lamport` (migration 058) refuses it instead — the loser is
...
test('the later lamport wins a ticket\'s covers, whichever push arrives first', opts, async () => {
  ...
  const before = await one("SELECT id, party_lamport AS version FROM ticket WHERE table_no = $1"
  ...
  const afterWin = await one('SELECT party, covers, party_lamport AS version FROM ticket WHERE id = $1', [before.id]);
  ...
  const afterLose = await one('SELECT party, covers, party_lamport AS version FROM ticket WHERE id = $1', [before.id]);
  ...
  const afterOld = await one('SELECT party, party_lamport AS version FROM ticket WHERE id = $1', [before.id]);
...
test('a scalar edit with no conflict applies exactly as before', opts, async () => {
  ...
  const row = await one('SELECT party, party_lamport AS version FROM ticket WHERE id = $1', [t.id]);
  assert.strictEqual(Number(row.party), 4);
  assert.strictEqual(Number(row.version), 50);

  // EACH FIELD HAS ITS OWN STAMP. An edit to a DIFFERENT field carrying a
  // lower lamport is not a conflict — nobody else touched the note — so it
  // applies. One stamp per ticket refused it and dropped a real edit.
  const n = await push([{ opId: uuid(), kind: 'ticket_status', lamport: 10,
    payload: { ticketId: t.id, note: 'window seat' } }]);
  assert.ok(!n.body.results[0].error, JSON.stringify(n.body));
  assert.strictEqual(n.body.results[0].result.ok, true,
    'a lower-lamport edit to another field is not refused by the covers stamp');
  const row2 = await one('SELECT note, party, note_lamport FROM ticket WHERE id = $1', [t.id]);
  assert.strictEqual(row2.note, 'window seat');
  assert.strictEqual(Number(row2.party), 4, 'and the covers are untouched');
  assert.strictEqual(Number(row2.note_lamport), 10);
});
...
test('web push: an owner granted this outlet, not homed here, gets till alerts', opts, async () => {
  ...
  const notify = require('../src/notify');
  await notify.notifyTill(outletId, { title: 'Bill asked', body: 'Table 4 is asking for the bill' });
  assert.strictEqual(stub.hits.length, 1, 'the group owner\'s device was woken');
  await stub.close();
});

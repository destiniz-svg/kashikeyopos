(function () {
  ...
  const state = { boot: null, loading: false };
  ...
  async function loadBoot() {
    ...
    try {
      ...
      if (res.ok) state.boot = await res.json();
    } catch (err) {
      state.boot = null;
    } finally {
      state.loading = false;
  ...
  function renderSheet(boot) {
    ...
    const orderRows = orders.slice(0, 5).map((order) => {
      ...
      return `<li><span>${esc(no)}</span><b>${esc(status)}</b><em>${esc(total)}</em></li>`;
    ...
    node.innerHTML = `
      <style>
        ...
        #kashikeyo-guest-profile-sheet h2{margin:0;font-size:24px;line-height:1.2;letter-spacing:0}
        #kashikeyo-guest-profile-sheet p{margin:5px 0 0;color:#a7b3c7;font-size:14px}
        #kashikeyo-guest-profile-sheet button{border:0;border-radius:14px;padding:13px 16px;background:#1f2d41;color:#f8fafc;font-weight:800;font-size:15px}
        #kashikeyo-guest-profile-sheet [data-kg-profile-close]{width:44px;height:44px;padding:0;border-radius:999px;font-size:26px;line-height:1;background:#1f2d41}
        #kashikeyo-guest-profile-sheet .kg-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:12px 0 18px}
        #kashikeyo-guest-profile-sheet .kg-stat{background:#162235;border:1px solid rgba(148,163,184,.18);border-radius:14px;padding:12px;min-width:0}
        #kashikeyo-guest-profile-sheet .kg-stat span{display:block;color:#93a4bc;font-size:12px;text-transform:uppercase;letter-spacing:.05em}#kashikeyo-guest-profile-sheet .kg-stat b{display:block;margin-top:5px;font-size:18px;overflow:hidden;text-overflow:ellipsis}
        #kashikeyo-guest-profile-sheet .kg-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:18px}#kashikeyo-guest-profile-sheet .kg-actions button:first-child{background:#06bfd4;color:#02111a}
        ...
        @media(max-width:560px){#kashikeyo-guest-profile-sheet .kg-panel{padding:18px}#kashikeyo-guest-profile-sheet .kg-stats{grid-template-columns:repeat(2,1fr)}#kashikeyo-guest-profile-sheet .kg-actions{grid-template-columns:1fr}#kashikeyo-guest-profile-sheet li{grid-template-columns:1fr auto}}
      </style>
  ...
  window.KashikeyoGuestProfile = { show: showProfile, context: guestContext };
})();

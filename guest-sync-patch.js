...
.ksh-custsel-list button{transition:background .12s ease}
.ksh-custsel-list button:hover{background:var(--k-panel2)}
`.replace(/\n/g, "");
...
const catNavJs =
  "window.__ksCatEmoji={'All':'🍽️','Main Dishes':'🍛','Coffee':'☕','Drinks':'🥤','Bakery':'🥐','Grocery':'🛒','Hedhikaa':'🍢','More':'⋯'};" +
  "window.__ksCatLbl=function(v,fallback){var e=(window.__ksCatEmoji||{})[v];return e?e+' '+fallback:fallback;};" +
  ...
  "window.__ksChipRow=function(h,items,isA,on,onc,ch,pad,bb){bb=bb||'px-3.5 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ';return h.jsx('div',{className:'flex overflow-x-auto',style:{gap:'12px',padding:pad||'8px 0'},children:items.map(function(it){return h.jsx('button',{onClick:function(){on(it.v);},className:bb+(isA(it.v)?onc:ch),children:it.label},String(it.v)+'|'+it.label);})});};" +
  "window.__ksCatRows=function(h,o){var g=window.__ksGN(o.groups),onc=o.chipOn,ch=o.chip,sel=o.sel,on=o.onSel;if(!g.length){var pr=Object.keys(window.__ksPresent(o.present)).sort();var it=[{v:'All',label:'All'}].concat(pr.map(function(c){return{v:c,label:c};}));return window.__ksChipRow(h,it,function(v){return v===sel;},on,onc,ch,'12px 0');}var mains=window.__ksMains(o.groups,o.present);var am=(!sel||sel==='All')?'All':(mains.indexOf(sel)>=0?sel:(window.__ksMainOf(sel,o.groups)||(window.__ksSubsOf('More',o.groups,o.present).indexOf(sel)>=0?'More':'All')));var mi=[{v:'All',label:window.__ksCatLbl('All','All')}].concat(mains.map(function(m){return{v:m,label:window.__ksCatLbl(m,m)};}));var r1=window.__ksChipRow(h,mi,function(v){return v===am;},on,onc,ch,'10px 0 6px');var r2=null;if(am!=='All'){var subs=window.__ksSubsOf(am,o.groups,o.present);var si=[{v:am,label:'All'}].concat(subs.map(function(s){return{v:s,label:s};}));r2=window.__ksChipRow(h,si,function(v){return v===sel;},on,onc,ch,'2px 0 10px');}return h.jsxs('div',{children:[r1,r2]});};" +
  "window.__ksGuestNav=function(h,o){var g=window.__ksGN(o.groups),onc=o.chipOn,ch=o.chip,set=o.setSel,mc=o.mcat||'',sc=o.scat||'',BB='px-4 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap ';if(!g.length){var pr=Object.keys(window.__ksPresent(o.present)).sort();var it=[{v:'',label:'All'}].concat(pr.map(function(c){return{v:c,label:c};}));return window.__ksChipRow(h,it,function(v){return v===mc;},function(v){set(v,'');},onc,ch,'0 0 8px',BB);}var mains=window.__ksMains(o.groups,o.present);var mi=[{v:'',label:window.__ksCatLbl('All','All')}].concat(mains.map(function(m){return{v:m,label:window.__ksCatLbl(m,m)};}));var r1=window.__ksChipRow(h,mi,function(v){return v===mc;},function(v){set(v,'');},onc,ch,'0 0 6px',BB);var r2=null;if(mc){var subs=window.__ksSubsOf(mc,o.groups,o.present);var si=[{v:'',label:'All'}].concat(subs.map(function(s){return{v:s,label:s};}));r2=window.__ksChipRow(h,si,function(v){return v===sc;},function(v){set(mc,v);},onc,ch,'2px 0 8px',BB);}return h.jsxs('div',{style:{marginBottom:'12px'},children:[r1,r2]});};";
...
/* Idle auto-lock: when the current outlet sets an idle timeout (idleLockSec) and a
   ...
   seconds without GENUINE interaction. Passive mouse movement (mousemove /
   pointermove) is deliberately ignored — only taps/clicks, key presses, touches
   and scrolls count as activity — so a drifting or bumped cursor can't hold the
   till open. Fully self-contained (no React surgery): tracks last activity, and
   every 2s checks the synced per-outlet pref (window.__ksCE / __ksStore) and the
   signed-in flag (window.__ksMe), calling the exposed lock (window.__ksLock).
   Does nothing at the PIN gate (no __ksMe). */
const idleLockJs =
...
patchFile(indexPath, (html) => html
  ...
  /* 123. Themed customer dropdown for the Settle modal. The native <select>
     renders an OS-styled option list that ignores the palette; replace it with a
     <details>-based dropdown (no extra React state needed) styled with the app
     tokens: a themed summary box + a rounded panel of customer rows (name +
     "owes" balance) with a check on the selection. Idempotent: the find is the
     original <select>, which the replacement no longer contains. */
  .replace(
...
patchFile(swPath, (sw) => sw.replace(/kashikeyo-2\.[0-9]\.\d+/g, "kashikeyo-2.9.87"));

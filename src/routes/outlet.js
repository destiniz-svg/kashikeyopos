...
r.post('/print', sameOutlet, atLeast('kitchen'), require('../printrelay'));
/* May this session print here? Asked by a store hub before it dials a printer
   on the shop's LAN: the hub holds no secret and no database, so the gate that
   guards the POST above is asked, not copied. */
r.get('/print', sameOutlet, atLeast('kitchen'), function (req, res) {
  res.json({ ok: true });
});

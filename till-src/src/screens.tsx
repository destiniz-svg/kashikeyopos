...
/* ── Orders — channel-categorized order tracking ─────────────────────────────
   One screen replaces the old separate Kitchen/QR Orders/Delivery tabs, split
   into the three channels staff actually think in: Walking & QR Tables (any
   dine-in bill, whether keyed at the register or placed from a table's QR),
   Delivery (POS-keyed call-ins and QR delivery orders alike — narrowing to
   QR-only would hide phone orders staff key in themselves), and Takeaway.
   Each category shows what's still just held (fired to nobody yet — tap to
   resume into the register) above the fired kitchen tickets (status filter +
   Bump, same lifecycle for every channel). "+ New" jumps into the register
   pre-set to that order type, since Register no longer has its own nav pill —
   Floor (tap a table) and here are the two doors into the order canvas. */
type OrdCat = "walk" | "deliv" | "away";
const ORD_CATS: [OrdCat, string, string][] = [
  ["walk", "Walking & QR Tables", "🍽️"],
  ["deliv", "Delivery", "🛵"],
  ["away", "Takeaway", "🥡"],
];
...
/* Same live-kitchen-status mapping the register's Open Bills rail and Floor
   tiles use, standalone here since this screen doesn't share App.tsx's
   closures. */
function kotStatusOf(b: any, orders: any[]): [string, string] {
  if (b.orderId) {
...
export function Orders({ onNewOrder, onResumeBill }: { onNewOrder: (otype: "takeaway" | "delivery") => void; onResumeBill: (b: any) => void }) {
  const st = useStore();
  const [cat, setCat] = useState<OrdCat>("walk");
  const [col, setCol] = useState("Active");
  const allOrders = st.byKind("orders").map((e) => e.data);
  const allParked = st.byKind("parked").map((e) => e.data);
  ...
  const orders = allOrders.filter((o) => catOf(o) === cat);
  /* Takeaway/Delivery have no Floor tile of their own, so their open bills —
     held or already fired — live here as the one resumable list (tap to jump
     back into the register, e.g. to add items or take payment). Dine-in bills
     keep Floor as their single source of truth (its table tiles already show
     held/in-kitchen state), so Walking shows kitchen tickets only. */
  const held = cat === "walk" ? [] : allParked.filter((b: any) => catOf(b) === cat);
  ...
  /* Badge = distinct in-flight items: active tickets, plus holds that haven't
     been fired yet (a fired bill is already counted once via its ticket — a
     parked entity lingers alongside it so it stays resumable, not so it's
     counted twice). */
  const catCount = (c: OrdCat) => {
    const activeTix = allOrders.filter((o) => catOf(o) === c && !["done", "delivered", "wasted"].includes(String(o.status || "new").toLowerCase())).length;
    const unfiredHolds = c === "walk" ? 0 : allParked.filter((b: any) => catOf(b) === c && !b.orderId).length;
    return activeTix + unfiredHolds;
  };
  ...
  return (
    <div style={X.scroll}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto" }}>
        {ORD_CATS.map(([id, label, icon]) => (
          <button key={id} onClick={() => { setCat(id); setCol("Active"); }} style={{ ...X.chip, minHeight: 48, ...(cat === id ? X.chipOn : {}) }}>{icon} {label} <span style={{ opacity: .7 }}>{catCount(id)}</span></button>
        ))}
      </div>
      {cat !== "walk" && (
        <button onClick={() => onNewOrder(cat === "deliv" ? "delivery" : "takeaway")} style={{ ...X.chip, borderColor: "var(--coral)", color: "var(--coral-text)", background: "var(--coralsoft)", marginBottom: 14 }}>
          + New {cat === "deliv" ? "delivery" : "takeaway"} order
        </button>
      )}
      {cat === "walk" && (
        <div style={{ background: "var(--greensoft)", borderRadius: 14, padding: "13px 15px", display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 14 }}>
          <span style={{ fontSize: 17 }}>▦</span>
          <div style={{ flex: 1, fontSize: 12.5, color: "var(--green)", fontWeight: 600, lineHeight: 1.45 }}>Print one QR per table — guests scan, order and pay from their phone, and it lands here tagged with the table, same as a walk-in seated at the register.
      ...
      {held.length > 0 && (
        <>
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".08em", color: "var(--ink3)", margin: "4px 0 8px" }}>OPEN BILLS · TAP TO RESUME</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
            {held.map((b: any) => {
              const items = (b.lines || []).reduce((a: number, l: any) => a + (l.qty || 0), 0);
              const [lbl, col2] = b.orderId ? kotStatusOf(b, allOrders) : ["Held", "var(--amber)"];
              return (
                <button key={b.id} onClick={() => onResumeBill(b)} style={{ ...X.card, ...X.tap, display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", textAlign: "left" }}>
                  <b style={{ fontSize: 13.5 }} className="num">{b.no || "#" + items}</b>
                  <span style={{ fontSize: 12.5, color: "var(--ink2)" }}>{items} items{b.custName ? " · " + b.custName : ""}{b.table ? " · " + b.table : ""}</span>
                  <div style={{ flex: 1 }} />
                  <span style={{ ...X.tag, background: "color-mix(in srgb," + col2 + " 16%,transparent)", color: col2 }}>{lbl}</span>
                </button>
              );
            })}
          </div>
      ...
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".08em", color: "var(--ink3)", margin: "4px 0 8px" }}>KITCHEN TICKETS</div>
      ...
      <div style={{ ...X.card, padding: 0, minHeight: 220 }}>
        ...
        </div> : <div style={{ padding: "50px 20px", textAlign: "center", color: "var(--ink3)" }}>
          <div style={{ fontSize: 28, marginBottom: 8 }}>🍳</div>No {ORD_CATS.find(([id]) => id === cat)![1].toLowerCase()} tickets right now.
        </div>}
      </div>
      {cat === "deliv" && (
        <div style={{ ...X.card, padding: 16, marginTop: 18 }}>

  ...
  return (
    <div style={G.wrap}>
      <div style={G.phone}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "8px 14px 90px", display: "flex", flexDirection: "column", gap: 9 }}>
          {items.map((p) => {
            const q = cart[p.id] || 0; const t = tintFor(p.cat); const so = p.soldOut;
            return (
              <div key={p.id} style={{ ...G.row, opacity: so ? .55 : 1, borderColor: q > 0 ? "var(--coral)" : "var(--line)" }}>
                <span style={{ ...G.rowGlyph, background: t[0], color: t[1] }}>{p.emoji || (p.name || "?")[0].toUpperCase()}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 13.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</div>
                  <div className="num" style={{ fontWeight: 800, fontSize: 12.5, color: "var(--ink3)", marginTop: 1 }}>{money(p.price)}{so && <span style={{ color: "var(--red)", marginInlineStart: 8, fontSize: 11, fontWeight: 700 }}>Sold out</span>}</div>
                </div>
                {so ? null : q > 0
...
const G: Record<string, React.CSSProperties> = {
  ...
  add: { width: 30, height: 30, borderRadius: 99, background: "var(--coralsoft)", color: "var(--coral)", fontSize: 18, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", flex: "0 0 30px" },
  stepper: { display: "inline-flex", alignItems: "center", gap: 4, background: "var(--coralsoft)", borderRadius: 999, padding: "3px 5px" },
  stepBtn: { width: 26, height: 26, borderRadius: 99, background: "var(--sur)", fontSize: 16, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center", lineHeight: 1 },
  /* Menu rows — the QR-portal design shown in the register's QR Orders tab:
     a soft card row with a tinted glyph, name + price, and a coral add. */
  row: { display: "flex", alignItems: "center", gap: 11, background: "var(--sur)", border: "1px solid var(--line)", borderRadius: 14, padding: "10px 12px", boxShadow: "var(--shadow)" },
  rowGlyph: { width: 36, height: 36, borderRadius: 11, display: "grid", placeItems: "center", fontWeight: 800, fontSize: 18, flex: "0 0 36px" },
